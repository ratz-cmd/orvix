...
const FSTREAM_BASE_URL_VAL = process.env.FSTREAM_BASE_URL || "https://french-stream.net/";
...
app.use('/api/vip', require('./routes/vipLicenseRoutes'));
...
// Extracteurs natifs embarqués (portage de l'extension Chrome) — ultra-léger,
// résout uniquement l'URL brute .m3u8/.mp4, la vidéo transite directement
// depuis le CDN de l'hébergeur vers le navigateur (0 proxy vidéo côté serveur).
app.use('/api/extract', require('./routes/nativeExtract'));
...
const appReady = (async () => {
  try {
    ...
    console.error("MySQL connection error (non-fatal in dev):", error.message);
  }
})();
