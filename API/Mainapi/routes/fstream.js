...
const FSTREAM_BASE_URL = process.env.FSTREAM_BASE_URL || 'https://french-stream.net';
...
function getAgentForProxy(entry) {
  if (!entry || entry.type === 'direct') return null;
...
async function fetchEpisodesFromStaticJs(pageUrl) {
  ...
  const proxies = [{ type: 'direct' }, ...getShuffledAllProxies()];
  const maxAttempts = Math.min(proxies.length, 4);
  let lastError = null;
...
async function extractFStreamPlayers(htmlContent, isSeries = false, pageUrl = null) {
  try {
    ...
    if (isSeries) {
      ...
      // Extraction via .option[data-url] (structure HTML moderne French-Stream : Vidzy, Uqload, VOE...)
      if (players.length === 0) {
        $('.option[data-url], [data-url], .movie-players .option').each((_, el) => {
          const $el = $(el);
          let url = $el.attr('data-url') || $el.attr('data-src') || $el.attr('href');
          if (!url || !url.startsWith('http')) return;
          const text = ($el.text() || '').toUpperCase();
          let version = 'Default';
          if (text.includes('TRUEFRENCH') || text.includes('VFF') || text.includes('FRENCH') || text.includes('VF')) {
            version = 'VFF';
          } else if (text.includes('VFQ')) {
            version = 'VFQ';
          } else if (text.includes('VOSTFR')) {
            version = 'VOSTFR';
          }
          let provider = 'Lecteur';
          if (url.includes('vidzy')) provider = 'Vidzy';
          else if (url.includes('uqload')) provider = 'Uqload';
          else if (url.includes('voe')) provider = 'Voe';
          else if (url.includes('dood')) provider = 'Dood';
          else if (url.includes('vidmoly')) provider = 'Vidmoly';
          else if (url.includes('fsvid')) provider = 'Fsvid';
          else if (url.includes('sibnet')) provider = 'Sibnet';

          players.push({ url, type: 'embed', quality: 'HD', player: provider, version });
        });
      }
