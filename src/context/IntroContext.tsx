...
export const IntroProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const introEnabled = localStorage.getItem('orvix_intro_enabled') === 'true';
    const hasSeenIntro = localStorage.getItem('orvix_intro_seen') === 'true';
  ...
  useEffect(() => {
    const handleIntroReset = () => {
      ...
      localStorage.removeItem('orvix_intro_seen');
    };
  ...
  const completeIntro = useCallback(() => {
    ...
    localStorage.setItem('orvix_intro_seen', 'true');
  }, []);
