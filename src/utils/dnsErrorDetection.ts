...
const ORIGIN_PROBE_PATH = '/orvix.png';
...
//
// Gate : on ne dispatch QUE si l'origine Orvix est elle-même injoignable. Si
// on arrive à charger /orvix.png, l'erreur vidéo vient du host vidéo tiers
// (source morte, CORS, proxy down…) et pas d'un blocage FAI sur Orvix —
...
export function notifyDnsBlocked(
  ...
  void (async () => {
    ...
    if (reachable) return;
    window.dispatchEvent(new CustomEvent('orvix:dns-blocked', { detail }));
  })();
}
