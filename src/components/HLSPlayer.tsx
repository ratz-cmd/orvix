...
import { useClientSourceAvailability } from '../hooks/useClientSourceAvailability';
...
const handle429Error = (
  ...
  retryTimeout = setTimeout(() => {
    ...
    if (hls && videoRef.current) {
      ...
      console.log(`▶ Restarted HLS loading from: ${targetTime}s`);
    }
    retryTimeout = null;
...
const HLSPlayer = forwardRef<HLSPlayerRef, HLSPlayerProps>(({
  ...
  const { availability, probeSource, markAsUnavailable } = useClientSourceAvailability(
    movieId ? 'movie' : 'tv',
    movieId || tvShowId,
    seasonNumber,
    episodeNumber
  ...
  useEffect(() => {
    const list = [
      { id: 'peachify', url: movieId ? `https://peachify.top/embed/movie/${movieId}?sub=French&accent=dc2626` : `https://peachify.top/embed/tv/${tvShowId}/${seasonNumber}/${episodeNumber}?sub=French&accent=dc2626` },
      { id: 'videasy', url: movieId ? `https://player.videasy.net/fr/movie/${movieId}` : `https://player.videasy.net/fr/tv/${tvShowId}/${seasonNumber}/${episodeNumber}` },
    ];
    list.forEach(item => {
      if (movieId || (tvShowId && seasonNumber && episodeNumber)) {
        probeSource(item.id, item.url);
      }
    });
  }, [movieId, tvShowId, seasonNumber, episodeNumber, probeSource]);
  ...
  const sourceGroups = useMemo(() => {
    ...
    // ==========================================
    // 1. LECTEURS 100% VF (AUDIO FRANÇAIS DOUBLÉ)
    // ==========================================

    // Add FStream sources (French-Stream VF/VFQ 1080p)
    ...
    // Add SeekStreaming sources as dedicated frontend players (VF HD 1080p)
    const seekStreamingSources = customSources.filter(isSeekStreamingEmbedUrl);
    const regularCustomSources = customSources.filter(s => !isSeekStreamingEmbedUrl(s));

    if (seekStreamingSources.length > 0) {
      seekStreamingSources.forEach((source, index) => {
        embedSources.push({
          type: 'seekstreaming',
          id: `seekstreaming_${index}`,
          label: seekStreamingSources.length > 1
            ? `SeekStreaming ${index + 1} (VF HD)`
            : 'SeekStreaming (VF HD)',
          url: source,
        });
      });
    ...
    // ==========================================
    // 2. LECTEURS DE SECOURS (VOSTFR / INTERNATIONAUX)
    // ==========================================

    // Videasy (Secours VOSTFR)
    if (movieId) {
      embedSources.push({
        type: 'videasy',
        id: 'videasy_main',
        label: 'Videasy (VOSTFR / Secours)',
        url: `https://player.videasy.net/movie/${movieId}`,
      });
    } else if (tvShowId && seasonNumber != null && episodeNumber != null) {
      embedSources.push({
        type: 'videasy',
        id: 'videasy_main',
        label: 'Videasy (VOSTFR / Secours)',
        url: `https://player.videasy.net/tv/${tvShowId}/${seasonNumber}/${episodeNumber}`,
      });
    }
    ...
    const orvixBackupList = [
      ...
      { id: 'superembed', label: 'SuperEmbed (Secours VO)' },
      { id: 'vidsrccc', label: 'VidSrc (Secours VO)' },
    ...
    orvixBackupList.forEach(item => {
      ...
      if (movieId) {
        ...
        else if (item.id === 'autoembed') finalUrl = `https://player.autoembed.cc/embed/movie/${movieId}`;
        else if (item.id === 'embedsu') finalUrl = `https://embed.su/embed/movie/${movieId}`;
        else if (item.id === 'superembed') finalUrl = `https://multiembed.mov/directstream.php?video_id=${movieId}&tmdb=1`;
        else if (item.id === 'vidsrccc') finalUrl = `https://vidsrc.cc/v2/embed/movie/${movieId}`;
        else if (item.id === '111movies') finalUrl = `https://111movies.net/movie/${movieId}`;
      } else if (tvShowId && seasonNumber != null && episodeNumber != null) {
        if (item.id === 'peachify') finalUrl = `https://peachify.top/embed/tv/${tvShowId}/${seasonNumber}/${episodeNumber}?sub=French&accent=dc2626`;
        else if (item.id === 'autoembed') finalUrl = `https://player.autoembed.cc/embed/tv/${tvShowId}/${seasonNumber}/${episodeNumber}`;
        else if (item.id === 'embedsu') finalUrl = `https://embed.su/embed/tv/${tvShowId}/${seasonNumber}/${episodeNumber}`;
        else if (item.id === 'superembed') finalUrl = `https://multiembed.mov/directstream.php?video_id=${tvShowId}&tmdb=1&s=${seasonNumber}&e=${episodeNumber}`;
        else if (item.id === 'vidsrccc') finalUrl = `https://vidsrc.cc/v2/embed/tv/${tvShowId}/${seasonNumber}/${episodeNumber}`;
        else if (item.id === '111movies') finalUrl = `https://111movies.net/tv/${tvShowId}/${seasonNumber}/${episodeNumber}`;
      }
      embedSources.push({
        type: item.id,
        id: item.id,
  ...
  const handleSourceChange = (sourceType: string, sourceId: string, sourceUrl: string) => {
    ...
    const embedTypes = ['videasy', 'frembed', 'custom', 'omega', 'coflix', 'vostfr', 'adfree', 'fstream', 'wiflix', 'j1f', 'swiftflow', 'viper', 'vox'];
  ...
  useEffect(() => {
    ...
    const handleLoadStart = () => {
      ...
      sourceTimeoutRef.current = setTimeout(() => {
        console.log('⏰ Source loading timeout after 8 seconds - triggering automatic source switch');
        requestHlsFallback();
      }, 8000); // 8 secondes timeout - bascule rapide sans blocage infini
    };
    ...
    // Si la vidéo a déjà des données préchargées (readyState >= 2), masquer le spinner immédiatement
    ...
    if (isMP4) {
      ...
      const hlsConfig = {
        ...
        autoStartLoad: true, // Démarrage immédiat des fragments sans deadlock
      };
      ...
      hls.on(Hls.Events.MANIFEST_PARSED, (_event, data) => {
        ...
        if (isServersicuro) {
          ...
          if (data.audioTracks && data.audioTracks.length > 0) {
            ...
            console.warn(' No audio tracks found in manifest');
          }
        ...
        if (autoPlay && playbackPolicy.shouldAutoplay) {
          safePlay(video).catch((e) => {
            console.warn('[HLSPlayer] Autoplay non silencieux bloqué par le navigateur, repli silencieux (muted):', e);
            video.muted = true;
            setIsMuted(true);
            safePlay(video).catch((errMuted) => {
              console.warn('[HLSPlayer] Autoplay muet également bloqué, déblocage des contrôles et du bouton Play:', errMuted);
              setIsLoading(false);
              setIsPaused(true);
            });
          });
      ...
      hls.on(Hls.Events.FRAG_LOADED, (_event, data) => {
        // Un fragment a été chargé, le flux fonctionne - annuler le timeout de changement de source et masquer le spinner
        clearSourceTimeout();
        clearBufferingTimeout();
        setIsLoading(false);
        setIsBuffering(false);
      ...
      hls.on('hlsBufferEmptyError' as any, () => {
        const isServersicuro = src.includes('serversicuro.cc');
        if (isServersicuro) {
          console.warn(' Buffer empty error for serversicuro - attempting recovery...');
          hls.startLoad();
        }
      });
        ...
        if (data.type === Hls.ErrorTypes.MEDIA_ERROR) {
            ...
            if (videoRef.current && hlsRef.current) {
              ...
              setTimeout(() => {
                if (hlsRef.current && videoRef.current) {
                  ...
                  console.log(`▶ Restarted HLS loading from: ${currentTime}s after buffer stalled error`);
                }
              }, 2000);
            }
            return;
          ...
          if (data.details === Hls.ErrorDetails.FRAG_PARSING_ERROR) {
            ...
            if (globalRetryCount < 4 && isEarlyError) {
              ...
              console.log(`⏭ Skipping problematic fragment at start (SN: ${failedFragSN})`);
              ...
              if (lateRetryCount < 2) {
                ...
                console.warn(`⏭ Skipping problematic fragment ${failedFragSN} at ${currentTime}s`);
                ...
                if (videoRef.current) {
                  ...
                  if (shouldSkip) {
                    ...
                    if (video.duration && skipTime < video.duration) {
                      ...
                      console.log(` Cannot skip beyond video duration (${video.duration}s), staying at current position`);
                    }
                  } else {
          ...
          if (data.details === Hls.ErrorDetails.BUFFER_ADD_CODEC_ERROR) {
            console.warn(' Codec error - trying to continue playback...');
        ...
        if (data.fatal) {
          ...
          if (data.type === Hls.ErrorTypes.MEDIA_ERROR) {
            ...
            if (currentAttempts < 2) {
              ...
              console.warn(' Max recovery attempts reached, switching source...');
  ...
  // Combined active video filter (OLED + Super Resolution GPU)
  const getVideoActiveFilter = useCallback(() => {
    ...
    if (!isMobile && isUserVip() && upscaleMode !== 'off') {
      if (upscaleMode === 'cas') {
        ...
        filters.push('url(#orvix-upscale-ultra) contrast(1.05) saturate(1.04)');
      }
    }

    // OLED filter
    if (videoOledMode !== 'off') {
      const oled = getVideoOledFilter();
      if (oled && oled !== 'none') {
        filters.push(oled);
      }
    }
    ...
    if (autoPlay) {
      ...
      if (playPromise !== undefined) {
        playPromise
          ...
          .catch(() => {
            // Si l'autoplay échoue (blocage navigateur), on arrête le spinner et on affiche les contrôles
            setIsPlaying(false);
            setIsLoading(false);
            setShowControls(true);
          });
      ...
      const handleError = async (event: Event) => {
        ...
        if (target.error) {
          ...
          if (target.error.code === 4 && !disableCrossOrigin) {
            try {
              console.log('🕵 verifying whether the media error is a 403');
                          ...
                          <div className="mb-2 flex items-stretch gap-2">
                            ...
                            >
                              <div className="min-w-0 flex flex-1 flex-col">
                                <span className={`${isActive ? 'text-red-600 font-medium' : 'text-white'} flex items-center gap-1.5 flex-wrap`}>
                                  {source.label}
                                  {topLevelForPin && __pinnedSourceId === topLevelForPin && (
                                    <span className="ml-2 text-xs text-amber-400 font-semibold">#1</span>
                                  )}
                                  {source.type === 'vostfr' && availability[source.id]?.status === 'available' && (
                                    <span className="ml-1 text-[10px] px-1.5 py-0.5 bg-emerald-900/80 border border-emerald-500/40 text-emerald-200 rounded font-medium">
                                      🟢 Disponible
                                    </span>
                                  )}
                                  {source.type === 'vostfr' && availability[source.id]?.status === 'checking' && (
                                    <span className="ml-1 text-[10px] px-1.5 py-0.5 bg-amber-900/60 border border-amber-500/40 text-amber-200 rounded animate-pulse">
                                      ⏳ Vérif...
                                    </span>
                                  )}
                                  {source.type === 'vostfr' && availability[source.id]?.status === 'unavailable' && (
                                    <span className="ml-1 text-[10px] px-1.5 py-0.5 bg-red-900/60 border border-red-500/40 text-red-300 rounded font-medium">
                                      🔴 Non trouvé
                                    </span>
                                  )}
                                </span>
                              ...
                              {showVostfrMenu && (
                                ...
                                >
                                  {[
                                        ...
                                        { id: 'videasy', label: 'Videasy VOSTFR (Anglais ST-FR)' },
                                  ]
                                    .filter((vostfrSource) => availability[vostfrSource.id]?.status !== 'unavailable')
                                    ...
                                    return (
                                      ...
                                      >
                                        <div className="flex items-center gap-1.5">
                                          ...
                                          {availability[vostfrSource.id]?.status === 'available' && (
                                            <span className="text-[10px] px-1.5 py-0.2 bg-emerald-900/80 border border-emerald-500/40 text-emerald-200 rounded font-medium">
                                              🟢 1080p
                                            </span>
                                          )}
                                        </div>
  ...
  return (
    ...
    >
      ...
      {!isCasting && (
        ...
        >
          {/* SVG Convolve Kernels for Super Resolution VIP (PC GPU) */}
          <svg className="absolute w-0 h-0 pointer-events-none opacity-0" aria-hidden="true" style={{ position: 'absolute', width: 0, height: 0 }}>
            <defs>
              <filter id="orvix-upscale-cas" x="0%" y="0%" width="100%" height="100%">
                <feConvolveMatrix
                  order="3,3"
                  kernelMatrix="0 -0.5 0  -0.5 3.0 -0.5  0 -0.5 0"
                  preserveAlpha="true"
                />
              </filter>
              <filter id="orvix-upscale-ultra" x="0%" y="0%" width="100%" height="100%">
                <feConvolveMatrix
                  order="3,3"
                  kernelMatrix="-0.15 -0.6 -0.15  -0.6 4.0 -0.6  -0.15 -0.6 -0.15"
                  preserveAlpha="true"
                />
              </filter>
            </defs>
                    ...
                    >
                      ...
                      {(airPlayAvailable || castAvailable) && (
                        <div className="mb-3 p-2 bg-blue-900/30 border border-blue-700/50 rounded text-xs text-blue-200">
                          <div className="font-medium mb-1"> {t('watch.importantInfo')}</div>
                          {airPlayAvailable && !castAvailable ? (
                            <>
      ...
      <AnimatePresence>
        ...
        <HLSPlayerSettingsPanel
          ...
          {...{
            ...
            priorityCategory,
            upscaleMode,
            onUpscaleModeChange: handleUpscaleModeChange,
            isMobile,
          }}
        />
        )}
            ...
            >
              <div className="flex items-center justify-between mb-5">
                <h3 className="text-lg font-bold text-white flex items-center gap-2">
                  <span className="text-xl">⌨</span> {t('watch.keyboardShortcuts')}
                </h3>
