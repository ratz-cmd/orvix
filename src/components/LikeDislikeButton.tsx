...
const MIN_ORVIX_RATING_VOTES = 50;

export const calculateLikeDislikeRating = ({ likes, dislikes }: LikeDislikeStats): number | null => {
  const totalVotes = likes + dislikes;
  if (likes < MIN_ORVIX_RATING_VOTES && dislikes < MIN_ORVIX_RATING_VOTES) {
    return null;
  }
    ...
    <div className="flex flex-col items-start gap-2">
      ...
      <motion.button
        ...
        className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-all duration-300 ${userVote === 'dislike'
          ? 'bg-blue-500/30 text-blue-300 shadow-lg shadow-red-500/20'
          : 'bg-white/10 text-gray-300 hover:bg-white/20 hover:text-white'
          } disabled:opacity-50 disabled:cursor-not-allowed`}
        whileHover={{ scale: 1.05 }}
        whileTap={{ scale: 0.95 }}
