...
const POSTER_FALLBACK = `data:image/svg+xml,${encodeURIComponent('<svg width="500" height="750" xmlns="http://www.w3.org/2000/svg"><rect width="100%" height="100%" fill="#111"/><text x="50%" y="50%" fill="#444" font-size="36" font-family="sans-serif" text-anchor="middle" dy=".3em">ORVIX</text></svg>')}`;
...
const LazyImage: React.FC<LazyImageProps> = ({
  ...
  style,
  onError,
  placeholder = 'data:image/svg+xml;utf8,<svg width="342" height="513" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 342 513" preserveAspectRatio="xMidYMid meet"><rect width="100%" height="100%" fill="%23333"/><text x="50%" y="50%" fill="%23ccc" font-size="38" font-family="Arial, sans-serif" text-anchor="middle" dy=".3em">ORVIX</text></svg>',
  draggable = false,
  priority = false
}) => {
    ...
    <div className="embla-slide flex-none relative w-[144px] md:w-[192px]">
      ...
      >
        ...
        {isHistory && 'currentEpisode' in item && item.currentEpisode && item.media_type === 'tv' && (
          <span className="absolute top-9 left-2 z-10 px-2 py-1 rounded-lg bg-blue-600 text-[10px] font-semibold tracking-wider text-white">
        ...
        {isHistory && onRemoveItem ? (
          <button
            ...
            className="absolute top-2 right-2 z-20 p-2 rounded-full bg-blue-600/95 hover:bg-blue-600 active:scale-[0.85] text-white transition-[colors,transform] duration-150"
          >
    ...
    <div ref={rowRef} className="mb-4 content-row-container -mx-3 md:-mx-4 group/carousel" style={{ position: 'relative' }}>
        <div className="flex justify-between items-center mb-2 px-4 md:px-6 relative">
          ...
          {isHistory && onRemoveAll && limitedItems.length > 0 && (
            <button
              onClick={onRemoveAll}
              className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-700/80 hover:bg-blue-700 text-white text-xs font-medium rounded-full transition-colors"
        ...
        <div className="relative w-full overflow-visible">
          ...
          <button
            ...
            className={`hidden md:flex absolute left-6 md:left-8 top-1/2 z-[950]
                     ...
                     ring-1 ring-white/10 hover:ring-blue-500/40
