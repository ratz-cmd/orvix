...
const TVShows: React.FC = () => {
  ...
  const fetchTVShows = async () => {
    try {
      ...
      const cachedData = sessionStorage.getItem('orvix_tvshows_data');
      const cacheTimestamp = sessionStorage.getItem('orvix_tvshows_data_timestamp');
      ...
      sessionStorage.setItem('orvix_tvshows_data', JSON.stringify(cacheData));
      sessionStorage.setItem('orvix_tvshows_data_timestamp', Date.now().toString());
    } catch (error) {
  ...
  useEffect(() => {
    const cacheKey = 'orvix_tv_genre_images';
    const cacheTsKey = 'orvix_tv_genre_images_ts';
  ...
  useEffect(() => {
    ...
    document.title = `${t('tvShows.title')} — Orvix | Séries TV en Streaming VF & VOSTFR`;
  }, []);
