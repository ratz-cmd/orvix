            ...
            {displaySeasons.map((season) => (
              <button
                ...
                className={`w-full text-left px-4 py-3 text-sm transition-colors duration-150 ${
                  isDarkiWorld 
                    ? (selectedDarkiWorldSeason?.id === (season as DarkiWorldSeason).id ? 'bg-blue-600/20 text-blue-400' : 'text-gray-200 hover:bg-gray-700')
                    : (selectedSeason === (season as Season).season_number ? 'bg-blue-600/20 text-blue-400' : 'text-gray-200 hover:bg-gray-700')
                }`}
              >
                {isDarkiWorld
          ...
          >
            {displayEpisodes.map((episode) => (
              <button
                ...
                className={`w-full text-left px-4 py-3 text-sm transition-colors duration-150 ${
                  isDarkiWorld 
                    ? (selectedDarkiWorldEpisode?.id === (episode as DarkiWorldEpisode).id ? 'bg-blue-600/20 text-blue-400' : 'text-gray-200 hover:bg-gray-700')
                    : (selectedEpisode === (episode as Episode).episode_number ? 'bg-blue-600/20 text-blue-400' : 'text-gray-200 hover:bg-gray-700')
                }`}
              >
                {isDarkiWorld
              ...
              {selectedLink && (
                 <div className="mt-4 sm:mt-6 p-3 sm:p-4 bg-gray-800/50 rounded-xl border border-gray-700/50">
                  ...
                  {isDecoding ? (
                    ...
                    <div className="flex items-center text-blue-400 text-sm sm:text-base">
                      <div className="flex items-center">
...
const DownloadPage: React.FC = () => {
  ...
  // Fonction pour trier les liens selon l'ordre de priorité (Orvix en premier)
  const sortDownloadLinks = (links: DownloadLink[]): DownloadLink[] => {
    const orvix = links.filter(l => l.source === 'orvix');
    const rest = links.filter(l => l.source !== 'orvix');
  ...
  const proceedDecode = async (link: DownloadLink) => {
    ...
    // Les liens Orvix sont déjà des URLs directes (1fichier, Mega, …) ajoutées
    // côté admin : pas de décodage Darkino nécessaire.
    if (link.source === 'orvix' && link.url) {
      setIsDecoding(false);
      setDecodedLink({
        success: true,
        id: link.id,
        provider: link.provider || 'orvix',
        embed_url: link.url,
        metadata: {
          language: link.language,
          quality: link.quality,
            ...
            <div className="lg:col-span-2">
              <div className="bg-gray-800/50 rounded-xl p-4 sm:p-6 border border-gray-700/50">
                      ...
                      <div className="flex items-center gap-2">
                        <div className="relative">
                          <input
                            ...
                            min="1"
                            className="w-32 px-3 py-2 pr-8 bg-white/5 border border-gray-700/50 rounded-xl text-white placeholder-gray-400 focus:outline-none focus:border-blue-500/50 transition-colors [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                            onKeyPress={(e) => {
                              if (e.key === 'Enter') {
                ...
                <a
                  ...
                  className="mt-4 sm:mt-6 flex items-center gap-3 sm:gap-4 p-3 sm:p-4 bg-gray-800/50 rounded-xl border border-gray-700/50 hover:border-blue-500/50 transition-colors cursor-pointer group"
                >
                  <div className="flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 rounded-lg bg-blue-600/10 border border-blue-500/20 shrink-0">
                    <Download className="w-5 h-5 sm:w-6 sm:h-6 text-blue-500" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      <span className="text-sm sm:text-base font-semibold text-white group-hover:text-blue-400 transition-colors">{t('download.adTitle')}</span>
                      <span className="px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-wider rounded bg-blue-500/10 text-blue-400 border border-blue-500/20">{t('download.adTag')}</span>
                    </div>
                ...
                <div className="mt-4 sm:mt-6">
                  <button
                    ...
                    disabled={loadingLinks}
                    className="w-full flex items-center justify-center gap-2 px-4 sm:px-6 py-3 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-600 rounded-xl transition-colors text-sm sm:text-base"
                  >
                    {loadingLinks ? (
                  ...
                  <div className="mt-4 sm:mt-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                      {sortDownloadLinks(downloadLinks).map((link) => (
                        <div
                          key={link.id}
                          className="p-3 sm:p-4 bg-gray-800/50 rounded-xl border border-gray-700/50 cursor-pointer hover:border-blue-500/50 transition-colors duration-200"
                ...
                {error && (
                  <div className="mt-4 p-3 sm:p-4 bg-blue-900/20 border border-blue-500/50 rounded-xl backdrop-blur-sm">
