...
const TMDB_DETAILS_STORAGE_KEY = 'orvix_tmdb_details_cache_v1';
const TMDB_DETAILS_STORAGE_KEY_LEGACY = 'orvix_tmdb_details_cache_v1';
...
const getHomeCacheKeys = (language: string) => {
  ...
  return {
    data: `orvix_home_data_${suffix}`,
    timestamp: `orvix_home_data_${suffix}_timestamp`,
  };
};
...
const hydrateTmdbDetailsCache = () => {
  ...
  try {
    const raw = sessionStorage.getItem(TMDB_DETAILS_STORAGE_KEY) || sessionStorage.getItem(TMDB_DETAILS_STORAGE_KEY_LEGACY);
    if (!raw) return;
...
const Home: React.FC = () => {
  ...
  const fetchSagaCollections = useCallback(async () => {
    try {
      const cacheKey = `orvix_sagas_data_${tmdbLanguage.toLowerCase()}`;
      const cacheTsKey = `orvix_sagas_data_${tmdbLanguage.toLowerCase()}_ts`;
  ...
  useEffect(() => {
    ...
    document.title = `${t('nav.home')} — Orvix | Films et Séries en Streaming HD`;
    ...
    const structuredData = {
      ...
      "description": "Orvix - Plateforme de streaming gratuite de films et séries en HD 1080p (VF et VOSTFR). Regardez en ligne sans inscription."
    };
  ...
  const yourHistoryTitle = useMemo(
    () => <CarouselTitle icon="" iconClass="text-blue-500" label={t('home.yourHistory')} />,
    [t]
  );

  const trendingTodayTitle = useMemo(
    () => <CarouselTitle icon="" iconClass="text-blue-600" label={t('home.trendingToday')} />,
    [t]
  );
                    ...
                    >
                      ...
                      >
                        ...
                        <div className="flex items-start justify-center flex-col h-full z-20 relative gap-5 px-6 md:px-12 py-10">
                          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-600/90 border border-blue-500/40 text-white text-xs font-semibold uppercase tracking-wider">
