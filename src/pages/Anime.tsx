...
const CONTENT_RATING_CACHE_PREFIX = 'orvix_anime_content_rating_';
...
const Anime: React.FC = () => {
  ...
  const dataCacheKey = `orvix_anime_data_v2_${tmdbLanguage}`;
  const dataCacheTsKey = `${dataCacheKey}_timestamp`;
  const genreImageCacheKey = `orvix_anime_genre_images_v2_${tmdbLanguage}`;
  ...
  useEffect(() => {
    document.title = `${t('animePage.title')} — Orvix`;
  }, [i18n.language, t]);
        ...
        {topContent.length > 0 && (
          <div className="px-4 md:px-8">
            <LazySection index={0} immediateLoadCount={IMMEDIATE_LOAD_COUNT}>
              <EmblaCarousel
                title={<span><span className="text-blue-600 mr-2"></span><span>{t('animePage.trending')}</span></span>}
