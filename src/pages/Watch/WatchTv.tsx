...
type PlayerSourceType = 'videasy' | 'peachify' | 'autoembed' | 'embedsu' | 'superembed' | 'vidsrccc' | '111movies' | 'darkino' | 'mp4' | 'm3u8' | 'frembed' | 'custom' | 'vostfr' | 'omega' | 'coflix' | 'fstream' | 'wiflix' | 'j1f' | 'swiftflow' | 'viper' | 'vox' | string | number; // Allow string for embed types
...
const checkFrembedAvailability = async (showId: string, seasonNumber: number, episodeNumber: number): Promise<boolean> => {
  try {
    ...
    const response = await axios.get(checkUrl, { timeout: 2500 });

    // Check status and if result has items
    return response.data?.status === 200 && Boolean(
      (response.data?.result?.total ?? 0) > 0 ||
      (response.data?.result?.totalItems ?? 0) > 0 ||
      (Array.isArray(response.data?.result?.items) && response.data.result.items.length > 0)
    );
...
const WatchTv: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (!areSourcesLoading) {
      ...
      if (
        ...
        // Try videasy as a fallback
        else if (id) {
          setSelectedSource('videasy');
          setVideoSource(null);
          setEmbedUrl(`https://player.videasy.net/tv/${id}/${seasonNumber}/${episodeNumber}`);
    ...
    const initialFetch = async () => {
      ...
      try {
        ...
        if (fstreamHlsSources.length > 0) {
          ...
          console.log(` Added ${fstreamHlsSources.length} FStream TV HLS sources to final sources (HD prioritized, Vidzy/Uqload as fallback)`);
        }
        ...
        const applyEmbedConfig = (sourceId: TopLevelSourceId): boolean => {
          switch (sourceId) {
            case 'videasy': {
              console.log('🎬 Sélection de VIDEASY 1080p VF (Série)');
              const videasyUrl = `https://player.videasy.net/tv/${id}/${seasonNumber}/${episodeNumber}`;
              setSelectedSource('videasy');
            ...
            case 'seekstreaming': {
              const seekUrl = (customLinksResult.customLinks || []).find(isSeekStreamingEmbedUrl);
              if (!seekUrl) return false;
              console.log(' Selecting SEEKSTREAMING as primary source:', seekUrl);
            ...
            case 'peachify': {
              const url = `https://peachify.top/embed/tv/${id}/${seasonNumber}/${episodeNumber}?sub=French&accent=dc2626`;
              setSelectedSource('peachify');
              setVideoSource(null);
              setEmbedUrl(url);
              setEmbedType('peachify');
              currentSourceRef.current = 'peachify';
              setOnlyVostfrAvailable(false);
              return true;
            }
            case 'autoembed': {
              const url = `https://player.autoembed.cc/embed/tv/${id}/${seasonNumber}/${episodeNumber}`;
              setSelectedSource('autoembed');
              setVideoSource(null);
              setEmbedUrl(url);
              setEmbedType('autoembed');
              currentSourceRef.current = 'autoembed';
              setOnlyVostfrAvailable(false);
              return true;
            }
            case 'embedsu': {
              const url = `https://embed.su/embed/tv/${id}/${seasonNumber}/${episodeNumber}`;
              setSelectedSource('embedsu');
              setVideoSource(null);
              setEmbedUrl(url);
              setEmbedType('embedsu');
              currentSourceRef.current = 'embedsu';
              setOnlyVostfrAvailable(false);
              return true;
            }
            case 'superembed': {
              const url = `https://multiembed.mov/directstream.php?video_id=${id}&tmdb=1&s=${seasonNumber}&e=${episodeNumber}`;
              setSelectedSource('superembed');
              setVideoSource(null);
              setEmbedUrl(url);
              setEmbedType('superembed');
              currentSourceRef.current = 'superembed';
              setOnlyVostfrAvailable(false);
              return true;
            }
            case 'vidsrccc': {
              const url = `https://vidsrc.cc/v2/embed/tv/${id}/${seasonNumber}/${episodeNumber}`;
              setSelectedSource('vidsrccc');
              setVideoSource(null);
              setEmbedUrl(url);
              setEmbedType('vidsrccc');
              currentSourceRef.current = 'vidsrccc';
              setOnlyVostfrAvailable(false);
              return true;
            }
            case '111movies': {
              const url = `https://111movies.net/tv/${id}/${seasonNumber}/${episodeNumber}`;
              setSelectedSource('111movies');
              setVideoSource(null);
              setEmbedUrl(url);
              setEmbedType('111movies');
        ...
        if (!embedseekPromoted) {
          // Priority-driven auto-select. Availability construite à partir des sources françaises VF en premier.
          const availability: SourceAvailability[] = [
            ...
            { id: 'seekstreaming', hasData: !!(customLinksResult.customLinks && customLinksResult.customLinks.some(isSeekStreamingEmbedUrl)) },
          ...
          if (!applied) {
            if (finalHlsSources && finalHlsSources.length > 0) {
              console.log('[WatchTv] 🛡️ Sauvetage auto-select sur sources HLS françaises:', finalHlsSources.length);
              setSelectedSource('nexus_hls');
              ...
              console.log('[WatchTv] 🛡️ Sauvetage auto-select sur FStream VF embed:', fstreamProcessedSources.length);
              setSelectedSource('fstream');
              setSelectedFstreamSource(0);
              ...
              setOnlyVostfrAvailable(false);
              setLoadingError(false);
            } else if (customLinksResult.customLinks && customLinksResult.customLinks.some(isSeekStreamingEmbedUrl)) {
              const seekUrl = customLinksResult.customLinks.find(isSeekStreamingEmbedUrl)!;
              console.log('[WatchTv] 🛡️ Sauvetage auto-select sur SeekStreaming VF:', seekUrl);
              setSelectedSource('seekstreaming');
              setEmbedUrl(seekUrl);
              setEmbedType('seekstreaming');
              setVideoSource(null);
              ...
              console.log('[WatchTv] 🛡️ Sauvetage auto-select sur Wiflix VF:', wiflixProcessedSources.length);
              setSelectedSource('wiflix');
              setSelectedWiflixSource(0);
              ...
              console.log('[WatchTv] 🛡️ Sauvetage de secours ultime sur Videasy');
              setSelectedSource('videasy');
              setVideoSource(null);
              setEmbedUrl(`https://player.videasy.net/tv/${id}/${seasonNumber}/${episodeNumber}`);
              setEmbedType('videasy');
        ...
        if (finalHlsSources && finalHlsSources.length > 0) {
          console.log('[WatchTv] 🛡️ Récupération d\'urgence sur les sources HLS déjà extraites:', finalHlsSources.length);
          setNexusHlsSources(finalHlsSources);
          setSelectedSource('nexus_hls');
          setSelectedNexusHlsSource(0);
          setVideoSource(finalHlsSources[0].url);
          ...
          console.log('[WatchTv] 🛡️ Récupération d\'urgence sur FStream embed:', fstreamProcessedSources.length);
          setSelectedSource('fstream');
          setSelectedFstreamSource(0);
          setEmbedUrl(fstreamProcessedSources[0].url);
          setEmbedType('fstream');
  ...
  useEffect(() => {
    const handleSourceChangeFromMenu = async (event: CustomEvent) => {
      ...
      else if (type === 'videasy' || type === 'peachify' || type === 'autoembed' || type === 'embedsu' || type === 'superembed' || type === 'vidsrccc' || type === '111movies' || type === 'custom' || type === 'vostfr' || type === 'omega' || type === 'coflix' || type === 'fstream' || type === 'wiflix' || type === 'j1f' || type === 'swiftflow' || type === 'viper' || type === 'adfree' || type === 'vox' || type === 'seekstreaming') {
  ...
  if (videoSource) {
    hlsSrc = videoSource;
  } else if (selectedSource === 'nexus_hls' && nexusHlsSources.length > selectedNexusHlsSource) {
  ...
  const handleHlsError = useCallback(() => {
    ...
    if (selectedSource === 'darkino') {
      ...
      // If no coflix, try videasy
      else if (id) {
        console.log(`No coflix sources available, switching to videasy 1080p VF`);
        handleSourceChange('videasy', String(id));
      }
    ...
    // Handle coflix source errors
      ...
      // If coflix fails, try videasy
      if (id) {
        console.log(`Coflix source failed, switching to videasy 1080p VF`);
        handleSourceChange('videasy', String(id));
      }
      // Note: Ne pas forcer vostfr automatiquement - laisser l'utilisateur choisir depuis le menu
      else {
  ...
  useEffect(() => {
    ...
    */
    ...
    const isSourceReady = Boolean(selectedSource) && (
      ...
      Boolean(embedUrl) ||
      (!loadingDarkino && !loadingCoflix && !loadingOmega && !loadingCustom && !loadingSibnet && !loadingFrembed && !loadingFstream && !loadingWiflix && !loadingVox)
    );
  ...
  // black screen). Rendered as overlay below so players pre-load in background.
  if (adPopupBypass) {
    return (
  ...
  return (
    <div style={{ minHeight: 'calc(var(--vh, 1vh) * 100)', overflow: 'hidden' }} className="w-full bg-black text-white overflow-hidden fixed inset-0">
      ...
      // choisir autre chose.
      ) : (['nexus_hls', 'nexus_file', 'darkino', 'mp4', 'm3u8', 'bravo', 'kisskh', 'swiftflux'].includes(selectedSource as string) && ((hlsSrc && hlsSrc.trim() !== '') || selectedSource === 'swiftflux' || Boolean(videoSource))) && (!adPopupTriggered || shouldLoadIframe || hasClickedAd) ? (
          ...
          <button
            onClick={() => {
              const targetUrl = embedType === 'vostfr'
                ? `https://player.videasy.net/fr/tv/${id}/${seasonNumber}/${episodeNumber}`
                : embedUrl || '';
