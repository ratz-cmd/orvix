...
import { useIframeAdGuard } from '../../utils/iframeAdGuard';
...
type PlayerSourceType = 'primary' | 'vostfr' | 'videasy' | 'peachify' | 'autoembed' | 'embedsu' | 'superembed' | 'vidsrccc' | '111movies' | 'vidsrcsu' | 'vidsrcwtf1' | 'vidsrcwtf5' | 'multi' | 'omega' | 'darkino' | 'mp4' | 'coflix' | 'frembed' | 'custom' | 'nexus_hls' | 'nexus_file' | 'fstream' | 'wiflix' | 'j1f' | 'swiftflow' | 'viper' | 'vidmoly' | 'dropload' | 'adfree' | 'bravo' | 'kisskh' | 'seekstreaming' | number;
...
const checkMovieAvailability = async (movieId: string) => {
  try {
    ...
    try {
      const frembedResponse = await axios.get(`${getFrembedBase()}/api/public/v1/movies/${movieId}`, { timeout: 2500 });
      console.log(`Checking Frembed for ID ${movieId}:`, frembedResponse.data);
      const isFrembedAvailable = frembedResponse.data?.status === 200 && Boolean(
        (frembedResponse.data?.result?.total ?? 0) > 0 ||
        (frembedResponse.data?.result?.totalItems ?? 0) > 0 ||
        (Array.isArray(frembedResponse.data?.result?.items) && frembedResponse.data.result.items.length > 0)
      );
...
const WatchMovie: React.FC = () => {
  ...
  // Bouclier anti-popups pour neutraliser les pubs des iframes résiduelles
  useIframeAdGuard(Boolean(embedUrl || selectedSource === 'vostfr' || selectedSource === 'frembed'));
  ...
  const [adAccepted, setAdAccepted] = useState(() => isUserVip());
  ...
  const fetchVideoSources = async () => {
    ...
    try {
      ...
      // Ajouter les sources HLS FStream aux sources finales avec tri par qualité HD
      if (fstreamHlsSources.length > 0) {
        // Séparer les sources HD (Fsvid HD) des flux basse qualité (Vidzy 360p/432p, Uqload)
        const fsvidHdSources = fstreamHlsSources.filter(s => s.label.toLowerCase().includes('fsvid'));
        const lowResSources = fstreamHlsSources.filter(s => !s.label.toLowerCase().includes('fsvid'));

        // Les flux HD passent en priorité absolue, les autres sources existantes (PurStream 1080p, etc.) au milieu, et Vidzy 432p en fallback
        finalHlsSources = [...fsvidHdSources, ...finalHlsSources, ...lowResSources];
        console.log(` Added ${fstreamHlsSources.length} FStream HLS sources to final sources (HD prioritized, Vidzy/Uqload as fallback)`);
      }
      ...
      const applyEmbedConfig = async (sourceId: TopLevelSourceId): Promise<boolean> => {
        switch (sourceId) {
          case 'videasy': {
            console.log('🎬 Sélection de VIDEASY 1080p VF (Film)');
            const videasyUrl = `https://player.videasy.net/movie/${id}`;
            setSelectedSource('videasy');
            setEmbedUrl(videasyUrl);
            setEmbedType('videasy');
            setVideoSource(null);
            currentSourceRef.current = 'videasy';
          ...
          case 'seekstreaming': {
            const seekUrl = customLinks.find(isSeekStreamingEmbedUrl);
            if (!seekUrl) return false;
            console.log(' Selecting SEEKSTREAMING as normal source:', seekUrl);
            setSelectedSource('seekstreaming');
            setEmbedUrl(seekUrl);
            setEmbedType('seekstreaming');
            setVideoSource(null);
            currentSourceRef.current = 'seekstreaming';
            setOnlyVostfrAvailable(false);
            return true;
          }
          case 'swiftflux': {
          ...
          case 'fstream': {
            ...
            const targetUrl = sortedFstreamLocal[0].url;
          ...
          case 'wiflix': {
            ...
            const targetUrl = sortedWiflixLocal[0].url;
          ...
          case 'j1f': {
            ...
            const targetUrl = sortedJ1fLocal[0].url;
          ...
          case 'swiftflow': {
            ...
            const targetUrl = sortedSwiftflowLocal[0].url;
          ...
          case 'peachify': {
            const url = `https://peachify.top/embed/movie/${id}?sub=French&accent=dc2626`;
            setSelectedSource('peachify' as any);
            setVideoSource(null);
            setEmbedUrl(url);
            setEmbedType('peachify' as any);
            currentSourceRef.current = 'peachify';
            setOnlyVostfrAvailable(false);
            return true;
          }
          case 'autoembed': {
            const url = `https://player.autoembed.cc/embed/movie/${id}`;
            setSelectedSource('autoembed' as any);
            setVideoSource(null);
            setEmbedUrl(url);
            setEmbedType('autoembed' as any);
            currentSourceRef.current = 'autoembed';
            setOnlyVostfrAvailable(false);
            return true;
          }
          case 'embedsu': {
            const url = `https://embed.su/embed/movie/${id}`;
            setSelectedSource('embedsu' as any);
            setVideoSource(null);
            setEmbedUrl(url);
            setEmbedType('embedsu' as any);
            currentSourceRef.current = 'embedsu';
            setOnlyVostfrAvailable(false);
            return true;
          }
          case 'superembed': {
            const url = `https://multiembed.mov/directstream.php?video_id=${id}&tmdb=1`;
            setSelectedSource('superembed' as any);
            setVideoSource(null);
            setEmbedUrl(url);
            setEmbedType('superembed' as any);
            currentSourceRef.current = 'superembed';
            setOnlyVostfrAvailable(false);
            return true;
          }
          case 'vidsrccc': {
            const url = `https://vidsrc.cc/v2/embed/movie/${id}`;
            setSelectedSource('vidsrccc' as any);
            setVideoSource(null);
            setEmbedUrl(url);
            setEmbedType('vidsrccc' as any);
            currentSourceRef.current = 'vidsrccc';
            setOnlyVostfrAvailable(false);
            return true;
          }
          case '111movies': {
            const url = `https://111movies.net/movie/${id}`;
            setSelectedSource('111movies' as any);
            setVideoSource(null);
            setEmbedUrl(url);
            setEmbedType('111movies' as any);
            currentSourceRef.current = '111movies';
            setOnlyVostfrAvailable(false);
            return true;
          }
          case 'frembed': {
          ...
          case 'viper': {
            ...
            const targetUrl = sortedViperLocal[0].url;
      ...
      // (même garde-fou Viblix que ci-dessus)
        ...
        // des sources françaises VF d'abord, et garde les embeds VO/VOSTFR en queue de secours.
        const availability: SourceAvailability[] = [
          // 1. Sources françaises 100% VF (Audio Français Doublé)
          { id: 'fstream', hasData: fstreamProcessedSources.length > 0 },
          { id: 'seekstreaming', hasData: customLinks.some(isSeekStreamingEmbedUrl) },
          ...
          // 2. Sources de secours anglophones / VOSTFR (utilisées uniquement si aucune VF n'existe ou si expressément épinglées)
          { id: 'videasy', hasData: true },
          { id: 'peachify', hasData: true },
          { id: 'autoembed', hasData: true },
          { id: 'embedsu', hasData: true },
          { id: 'superembed', hasData: true },
          { id: 'vidsrccc', hasData: true },
          { id: '111movies', hasData: true },
        ...
        // qui a `hasData: true`. Si applyEmbedConfig échoue, on retry avec le candidat suivant.
        let applied = false;
        ...
        if (!applied) {
          if (finalHlsSources && finalHlsSources.length > 0) {
            console.log('[WatchMovie] 🛡️ Sauvetage auto-select sur sources HLS françaises:', finalHlsSources.length);
            setSelectedSource('nexus_hls');
          ...
          } else if (fstreamProcessedSources && fstreamProcessedSources.length > 0) {
            console.log('[WatchMovie] 🛡️ Sauvetage auto-select sur FStream VF embed:', fstreamProcessedSources.length);
            setSelectedSource('fstream');
            setSelectedFstreamSource(0);
            setEmbedUrl(fstreamProcessedSources[0].url);
            setEmbedType('fstream');
            currentSourceRef.current = 'fstream';
            setOnlyVostfrAvailable(false);
            setLoadingError(false);
          } else if (customLinks.some(isSeekStreamingEmbedUrl)) {
            const seekUrl = customLinks.find(isSeekStreamingEmbedUrl)!;
            console.log('[WatchMovie] 🛡️ Sauvetage auto-select sur SeekStreaming VF:', seekUrl);
            setSelectedSource('seekstreaming');
            setEmbedUrl(seekUrl);
            setEmbedType('seekstreaming');
            setVideoSource(null);
            ...
            setOnlyVostfrAvailable(false);
            setLoadingError(false);
          } else if (wiflixProcessedSources && wiflixProcessedSources.length > 0) {
            console.log('[WatchMovie] 🛡️ Sauvetage auto-select sur Wiflix VF:', wiflixProcessedSources.length);
            setSelectedSource('wiflix');
            setSelectedWiflixSource(0);
            ...
            console.log('[WatchMovie] 🛡️ Sauvetage de secours ultime sur Videasy');
            setSelectedSource('videasy');
            setVideoSource(null);
      ...
      // TENTATIVE DE SAUVETAGE INTELLIGENT (évite le basculement aveugle en anglais 360p) :
      // Si des sources françaises HD ont déjà été extraites avant le crash, les exploiter immédiatement !
      let rescued = false;
      if (finalHlsSources && finalHlsSources.length > 0) {
        console.log('[WatchMovie] 🛡️ Récupération d\'urgence sur les sources HLS déjà extraites:', finalHlsSources.length);
        setNexusHlsSources(finalHlsSources);
        setSelectedSource('nexus_hls');
        setSelectedNexusHlsSource(0);
        setVideoSource(finalHlsSources[0].url);
        ...
        console.log('[WatchMovie] 🛡️ Récupération d\'urgence sur FStream embed:', fstreamProcessedSources.length);
        setSelectedSource('fstream');
        setSelectedFstreamSource(0);
        setEmbedUrl(fstreamProcessedSources[0].url);
        setEmbedType('fstream');
        ...
        console.log('[WatchMovie] 🛡️ Récupération d\'urgence sur Wiflix embed:', wiflixProcessedSources.length);
        setSelectedSource('wiflix');
        setSelectedWiflixSource(0);
        setEmbedUrl(wiflixProcessedSources[0].url);
        setEmbedType('wiflix');
      ...
      if (!rescued) {
        console.log('[WatchMovie] 🛡️ Sauvetage auto-select d\'urgence sur Videasy 1080p VF');
        setSelectedSource('videasy');
        setVideoSource(null);
        setEmbedUrl(`https://player.videasy.net/movie/${id}`);
        setEmbedType('videasy');
  ...
  useEffect(() => {
    ...
    const processSourceSelectionFromMenu = async (event: CustomEvent) => {
      ...
      else if (['videasy', 'peachify', 'autoembed', 'embedsu', 'superembed', 'vidsrccc', '111movies', 'custom', 'vostfr', 'omega', 'coflix', 'fstream', 'wiflix', 'j1f', 'swiftflow', 'viper', 'seekstreaming'].includes(type)) {
  ...
  useEffect(() => {
    // VIP temporairement désactivé pour afficher les pubs à tous
    /*
    ...
    */
    ...
    // Quand la source sélectionnée est prête (instantané pour embeds, ou quand les chargements sont terminés)
    const isSourceReady = Boolean(selectedSource) && (
      selectedSource === 'frembed' ||
      selectedSource === 'videasy' ||
      Boolean(embedUrl) ||
      (!loadingDarkino && !loadingCoflix && !loadingOmega && !loadingFrembed && !loadingFstream && !loadingExtractions)
    );

    if (isSourceReady && selectedSource && !adPopupTriggered) {
    ...
    <div style={{ minHeight: 'calc(var(--vh, 1vh) * 100)', overflow: 'hidden' }} className="w-full bg-black overflow-hidden fixed inset-0">
      ...
      {isLoading ? (
            ...
            <div className="mb-6">
              ...
              <div className="flex justify-center gap-4">
                ...
                <button
                  ...
                  className="px-6 py-3 bg-blue-800 hover:bg-blue-700 text-white rounded-lg transition-all duration-200 shadow-lg"
                >
        ...
        <div className="w-full h-full flex items-center justify-center relative">
          {/* Badge Lecteur Externe */}
          <div className="fixed top-6 left-28 z-[9999] flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-950/90 border border-amber-500/50 text-amber-300 text-xs font-semibold backdrop-blur-md shadow-lg pointer-events-none">
            <span className="w-2 h-2 rounded-full bg-amber-400"></span>
            Lecteur Externe (AdGuard Actif)
          </div>
          {/* Back to Info Button */}
          <button
            onClick={() => navigate(`/movie/${id}`)}
      ...
      ) : (['nexus_hls', 'nexus_file', 'darkino', 'mp4', 'bravo', 'kisskh'].includes(selectedSource as string) && ((Boolean(videoSource)) || (selectedSource === 'nexus_hls' && nexusHlsSources.length > 0) || (selectedSource === 'bravo' && purstreamSources.length > 0 && canUseBravo) || (selectedSource === 'kisskh' && kisskhSources.length > 0))) && (!adPopupTriggered || shouldLoadIframe || hasClickedAd) ? (
        <div className="w-full h-full flex items-center justify-center relative">
          {/* Badge Lecteur Orvix Natif (0 pub) */}
          <div className="fixed top-6 left-28 z-[9999] flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-emerald-950/90 border border-emerald-500/50 text-emerald-300 text-xs font-semibold backdrop-blur-md shadow-lg pointer-events-none">
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
            ✨ Lecteur Orvix Natif (0 Pub • VIP Super Résolution)
          </div>
          <HLSPlayer
            priorityCategory="moviesTv"
            autoFallbackGuard={autoFallbackGuard}
        ...
        <div className="w-full h-full flex flex-col items-center justify-center relative">
          ...
          <div className="fixed top-6 right-8 z-[10000] flex items-center gap-2">
            {/* Bouton Ouvrir dans une nouvelle page */}
            <button
              onClick={() => window.open(embedUrl || (selectedSource === 'vostfr' ? `https://player.videasy.net/fr/movie/${id}` : `https://player.videasy.net/movie/${id}`), '_blank', 'noopener')}
              className="flex items-center gap-2 px-3 py-2 rounded-lg bg-gray-800/90 border border-gray-600 hover:bg-gray-700/90 text-white font-medium text-sm transition-all duration-200"
              title={t('watch.openInNewPage')}
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
          ...
          <iframe
            src={embedUrl || (selectedSource === 'vostfr' ? `https://player.videasy.net/fr/movie/${id}` : `https://player.videasy.net/movie/${id}`)}
            className="w-full h-full border-0"
            allowFullScreen
