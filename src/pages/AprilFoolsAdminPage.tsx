...
const AprilFoolsAdminPage: React.FC = () => {
  ...
  const metrics = useMemo(
    () => [
      ...
      { label: t('aprilAdmin.metrics.priorityRequests'), value: '5', tone: 'text-blue-200' },
    ],
    [t]
  );
  ...
  const fakeInvoices = useMemo(
    () => [
      { id: 'vip_8A42R', amount: '49.99 EUR', payer: 'operator@orvix.local', status: t('aprilAdmin.status.confirming') },
      { id: 'vip_4P19M', amount: '5.00 EUR', payer: 'gift@orvix.local', status: t('aprilAdmin.status.partial') },
      { id: 'vip_7K21Q', amount: '99.00 EUR', payer: 'priority@orvix.local', status: t('aprilAdmin.status.readyToValidate') },
    ],
    [t]
  );
