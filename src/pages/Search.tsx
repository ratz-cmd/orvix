    ...
    return (
        <div className="flex justify-center items-center gap-1.5 flex-wrap my-6">
            <motion.button whileTap={{ scale: 0.92 }} onClick={() => onSelect(1)} disabled={activePage <= 1}
                className={`min-w-[40px] h-10 rounded-full text-sm font-medium transition-all ${activePage === 1 ? 'bg-blue-600 text-white' : 'bg-white/5 text-white/60 hover:bg-white/10 hover:text-white border border-white/10'} disabled:opacity-30`}>
                1
            </motion.button>
            ...
            {Array.from({ length: 5 }, (_, i) => {
                ...
                return p > 1 && p < maxPages ? (
                    <motion.button key={p} whileTap={{ scale: 0.92 }} onClick={() => onSelect(p)}
                        className={`min-w-[40px] h-10 rounded-full text-sm font-medium transition-all ${p === activePage ? 'bg-blue-600 text-white' : 'bg-white/5 text-white/60 hover:bg-white/10 hover:text-white border border-white/10'}`}>
                        {p}
                    </motion.button>
                ) : null;
            })}
            ...
            {maxPages > 1 && (
                <motion.button whileTap={{ scale: 0.92 }} onClick={() => onSelect(maxPages)} disabled={activePage >= maxPages}
                    className={`min-w-[40px] h-10 rounded-full text-sm font-medium transition-all ${activePage === maxPages ? 'bg-blue-600 text-white' : 'bg-white/5 text-white/60 hover:bg-white/10 hover:text-white border border-white/10'} disabled:opacity-30`}>
                    {maxPages}
                </motion.button>
            )}
        </div>
...
const Search: React.FC = () => {
    ...
    // Set simple title
    React.useEffect(() => {
        document.title = `${t('search.title')} — Orvix`;
    }, []);
            ...
            >
                ...
                >
                    ...
                    >
                        ...
                        >
                            <ShinyText text={t('search.searchMoviesAndShows')} speed={3} color="#ffffff" shineColor="#3b82f6" className="inline" />
                        </motion.h1>
                                        ...
                                        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.28 }} className="relative">
                                            ...
                                            <div className="relative">
                                                <input
                                                    ...
                                                    className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder:text-white/30 focus:outline-none focus:border-blue-500/50 transition-colors pr-10"
                                                />
                                                {keywordQuery && (
                                                    <button
                                                        type="button"
                                                ...
                                                <div className="flex flex-wrap gap-2 mt-3">
                                                    {selectedKeywords.map((keyword) => (
                                                        <button
                                                            ...
                                                            className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-blue-600/15 border border-blue-500/30 text-sm text-white hover:bg-blue-600/25 transition-colors"
                                                        >
                                    ...
                                    <Loader className="w-5 h-5 text-white opacity-40 animate-spin" />
                                        ...
                                        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="flex justify-center">
                                            <motion.button
                                                ...
                                                whileTap={{ scale: 0.98 }}
                                                className="w-full max-w-sm mx-auto py-3 rounded-2xl bg-blue-600 hover:bg-blue-500 text-white font-semibold flex items-center justify-center gap-2 transition-colors"
                                            >
                                                <SearchIcon size={20} />
