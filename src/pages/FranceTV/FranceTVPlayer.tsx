    ...
    >
      ...
      {error && (
        <div className="absolute inset-0 z-30 flex flex-col items-center justify-center bg-black/90 px-4">
          <AlertCircle className="w-14 h-14 text-blue-400 mb-4" />
          <p className="text-blue-400 text-center mb-6 max-w-lg text-sm">{error}</p>
          <button onClick={() => navigate(-1)}
          ...
          <div className="flex items-center gap-0.5 sm:gap-1">
            ...
            <motion.div
              animate={{
                ...
                color: isPipActive ? '#2563eb' : '#ffffff'
              }}
      ...
      >
        <div className="max-w-screen-2xl mx-auto">
          ...
          >
            ...
            {/* Handle */}
            <div
              className="absolute top-1/2 -translate-y-1/2 w-4 h-4 bg-blue-600 rounded-full -ml-2 opacity-0 group-hover:opacity-100 transition-opacity shadow-lg"
                  ...
                  <div className="overflow-hidden transition-all duration-200 flex items-center h-full w-0 group-hover/volume:w-[112px] ml-0 group-hover/volume:ml-2">
                    <div className="w-[100px] mx-[6px] flex items-center h-full">
                      <input
                        ...
                        className="w-full accent-blue-600 appearance-none h-1 rounded-full cursor-pointer"
                        style={{
                          background: `linear-gradient(to right, #2563eb ${(isMuted ? 0 : volume) * 100}%, rgba(255, 255, 255, 0.2) ${(isMuted ? 0 : volume) * 100}%)`
                        }}
                      />
                    </div>
                  </div>
                    ...
                    {qualities.map(q => (
                      <button
                        ...
                        className={`w-full text-left px-3 py-2.5 rounded-lg text-sm transition-colors cursor-pointer ${
                          selectedQuality === q.value ? 'bg-blue-600/20 text-blue-400' : 'text-gray-300 hover:bg-white/5'
                        }`}
                      >
                        {q.label}
                        {selectedQuality === q.value && <span className="float-right text-blue-400"></span>}
                      </button>
                    ))}
                  </motion.div>
                )}
                  ...
                  <motion.div key="audio" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} transition={{ duration: 0.25 }}>
                    {audioTracks.map(a => (
                      <button
                        ...
                        className={`w-full text-left px-3 py-2.5 rounded-lg text-sm transition-colors cursor-pointer ${
                          selectedAudio === a.value ? 'bg-blue-600/20 text-blue-400' : 'text-gray-300 hover:bg-white/5'
                        }`}
                      >
                        {a.label}
                        {selectedAudio === a.value && <span className="float-right text-blue-400"></span>}
                      </button>
                    ))}
                  </motion.div>
                )}
                  ...
                  <motion.div key="subtitles" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} transition={{ duration: 0.25 }}>
                    {subtitles.map(s => (
                      <button
                        ...
                        className={`w-full text-left px-3 py-2.5 rounded-lg text-sm transition-colors cursor-pointer ${
                          selectedSub === s.value ? 'bg-blue-600/20 text-blue-400' : 'text-gray-300 hover:bg-white/5'
                        }`}
                      >
                        {s.label}
                        {selectedSub === s.value && <span className="float-right text-blue-400"></span>}
                      </button>
                    ))}
                  </motion.div>
                )}
                  ...
                  <motion.div key="speed" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} transition={{ duration: 0.25 }}>
                    {[0.25, 0.5, 0.75, 1, 1.25, 1.5, 1.75, 2].map(speed => (
                      <button
                        ...
                        className={`w-full text-left px-3 py-2.5 rounded-lg text-sm transition-colors cursor-pointer ${
                          playbackSpeed === speed ? 'bg-blue-600/20 text-blue-400' : 'text-gray-300 hover:bg-white/5'
                        }`}
                      >
                        {speed === 1 ? t('watch.normalLabel') : `${speed}x`}
                        {playbackSpeed === speed && <span className="float-right text-blue-400"></span>}
                      </button>
                    ))}
                  </motion.div>
                )}
