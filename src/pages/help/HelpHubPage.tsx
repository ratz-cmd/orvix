...
const ACCENT_BORDER: Record<TutoAccent, string> = {
  red: 'border-blue-500/20 group-hover:border-blue-500/50',
...
const renderHighlighted = (text: string, query: string) => {
  ...
  return parts.map((part, i) =>
    part.toLowerCase() === query.toLowerCase() ? (
      <mark
        key={i}
        className="bg-blue-500/30 text-blue-100 rounded-sm px-0.5 font-semibold"
      >
        {part}
      </mark>
    ) : (
...
const HelpHubPage: React.FC = () => {
  ...
  useEffect(() => {
    document.title = `${t('help.hub.title')} — Orvix`;
  }, [t]);
    ...
    <SquareBackground className="min-h-screen">
      ...
      >
        ...
        <motion.div variants={itemVariants} className="mb-8">
          <div className="relative max-w-xl mx-auto">
            ...
            <input
              ...
              className="w-full rounded-xl border border-white/10 bg-white/[0.04] pl-10 pr-10 py-3 text-white placeholder:text-zinc-500 outline-none focus:border-blue-500/60 focus:bg-white/[0.06] transition-colors"
            />
            {query && (
              <button
                type="button"
        ...
        >
          ...
          <a
            href={import.meta.env.VITE_SUPPORT_TELEGRAM_URL || 'https://t.me/orvix_site'}
            target="_blank"
