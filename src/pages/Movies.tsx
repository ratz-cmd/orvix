...
const Movies: React.FC = () => {
  ...
  const fetchMovies = async (pageNumber: number) => {
    try {
      ...
      if (pageNumber === 1) {
        const cachedData = sessionStorage.getItem('orvix_movies_data');
        const cacheTimestamp = sessionStorage.getItem('orvix_movies_data_timestamp');
      ...
      if (pageNumber === 1) {
        ...
        sessionStorage.setItem('orvix_movies_data', JSON.stringify(cacheData));
        sessionStorage.setItem('orvix_movies_data_timestamp', Date.now().toString());
      }
    } catch (error) {
  ...
  useEffect(() => {
    const cacheKey = 'orvix_movie_genre_images';
    const cacheTsKey = 'orvix_movie_genre_images_ts';
  ...
  useEffect(() => {
    ...
    document.title = `${t('movies.title')} — Orvix | Films en Streaming HD VF & VOSTFR`;
  }, []);
  ...
  if (error) {
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="bg-blue-600/10 text-blue-600 px-6 py-4 rounded-lg">
          {error}
        </div>
      </div>
    );
