...
const LiveTV: React.FC = () => {
  ...
  // Orvix Wrapped 2026 - Track Live TV viewing time (only when channel is open)
  useWrappedTracker({
    mode: 'viewing',
    ...
    const fetchManifest = async () => {
      try {
        ...
        if (isExtensionAvailable()) {
          console.log("Using Orvix Extension");
          try {
    ...
    return (
      ...
      >
        <div className="relative aspect-video rounded-xl overflow-hidden bg-white/[0.02] border border-white/[0.04] group-hover:border-white/10 transition-all duration-300 group-hover:bg-white/[0.04]">
          ...
          <div className="absolute top-2 left-2 flex items-center gap-1 px-1.5 py-0.5 bg-blue-500 rounded text-[9px] font-bold tracking-wide uppercase">
  ...
  const renderIptvCategoryOption = (category: IptvCategory) => {
    ...
    return (
      ...
      >
        ...
        >
          <span className={cn('truncate pr-2', active ? 'text-blue-400' : 'text-white/70 group-hover:text-white/90')}>
      ...
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8">
                  ...
                  return (
                    <motion.button
                      ...
                      className={cn(
                        ...
                        isActive
                          ? 'bg-blue-500/15 text-blue-400 border-blue-500/30 shadow-sm shadow-red-500/10'
                          : isLocked
                    ...
                    <>
                      <button
                        ...
                        className={cn(
                          ...
                          iptvCategoryDropdownOpen
                            ? 'bg-white/[0.06] border-blue-500/30 ring-1 ring-blue-500/10'
                            : 'bg-white/[0.03] border-white/[0.06] hover:bg-white/[0.05] hover:border-white/10'
                        )}
                      >
                        <i className="bi bi-globe text-blue-400 shrink-0" />
        ...
        {error && !loadingCatalogs && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center py-24 gap-4">
            <div className="p-4 rounded-2xl bg-blue-500/[0.06] border border-blue-500/10">
              <Wifi className="w-8 h-8 text-blue-400 opacity-60" />
            </div>
