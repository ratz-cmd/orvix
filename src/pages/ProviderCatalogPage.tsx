...
const PaginationBar = ({ currentPage, maxPages, onSelect }: { currentPage: number; maxPages: number; onSelect: (n: number) => void }) => (
    <div className="flex justify-center items-center gap-1.5 flex-wrap my-8">
        <motion.button whileTap={{ scale: 0.92 }} onClick={() => onSelect(1)} disabled={currentPage <= 1}
            className={`min-w-[40px] h-10 rounded-full text-sm font-medium transition-all ${currentPage === 1 ? 'bg-blue-600 text-white' : 'bg-white/5 text-white/60 hover:bg-white/10 hover:text-white border border-white/10'} disabled:opacity-30`}>
            1
        </motion.button>
        ...
        {Array.from({ length: 5 }, (_, i) => {
            ...
            return p > 1 && p < maxPages ? (
                <motion.button key={p} whileTap={{ scale: 0.92 }} onClick={() => onSelect(p)}
                    className={`min-w-[40px] h-10 rounded-full text-sm font-medium transition-all ${p === currentPage ? 'bg-blue-600 text-white' : 'bg-white/5 text-white/60 hover:bg-white/10 hover:text-white border border-white/10'}`}>
                    {p}
                </motion.button>
            ) : null;
        })}
        ...
        {maxPages > 1 && (
            <motion.button whileTap={{ scale: 0.92 }} onClick={() => onSelect(maxPages)} disabled={currentPage >= maxPages}
                className={`min-w-[40px] h-10 rounded-full text-sm font-medium transition-all ${currentPage === maxPages ? 'bg-blue-600 text-white' : 'bg-white/5 text-white/60 hover:bg-white/10 hover:text-white border border-white/10'} disabled:opacity-30`}>
                {maxPages}
            </motion.button>
        )}
    </div>
            ...
            <SquareBackground squareSize={48} borderColor="rgba(239, 68, 68, 0.10)" mode="combined">
                <div className="min-h-screen flex justify-center items-center">
                    <div className="text-center">
                        <h1 className="text-2xl font-bold text-blue-500">{t('common.error')}</h1>
                        ...
                        <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => window.location.reload()}
                            className="mt-4 px-6 py-2 bg-blue-600 hover:bg-blue-500 rounded-full text-white font-medium transition-colors">
                            {t('common.retry')}
                        </motion.button>
                    </div>
                </div>
          ...
          <ArrowLeft className="w-5 h-5 text-white opacity-70" />
                            ...
                            <div className="flex gap-2">
                                <Link to={`/provider/${providerId}/movies${genreId ? `/${genreId}` : ''}`}
                                    className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all text-sm font-medium ${type === 'movies' ? 'bg-blue-600 text-white' : 'bg-white/5 text-white/60 border border-white/10 hover:bg-white/10'}`}>
                                    <Film className="w-4 h-4" /> {t('providerCatalog.films')}
                                </Link>
                                <Link to={`/provider/${providerId}/tv${genreId ? `/${genreId}` : ''}`}
                                    className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all text-sm font-medium ${type === 'tv' ? 'bg-blue-600 text-white' : 'bg-white/5 text-white/60 border border-white/10 hover:bg-white/10'}`}>
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <motion.button
                                className={`flex items-center gap-2 px-4 py-2.5 rounded-2xl transition-all ${isFilterOpen
                                    ...
                                    : 'bg-white/5 backdrop-blur-md border border-white/10 hover:border-blue-500/40 hover:bg-blue-500/10'}`}
                                ...
                                <div className="flex flex-col sm:flex-row gap-4 sm:items-end">
                                    ...
                                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="flex gap-2">
                                        <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setViewType('grid')}
                                            className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all ${viewType === 'grid' ? 'bg-blue-600 text-white' : 'bg-white/5 text-white/60 border border-white/10 hover:bg-white/10'}`}>
                                        ...
                                        <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setViewType('list')}
                                            className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all ${viewType === 'list' ? 'bg-blue-600 text-white' : 'bg-white/5 text-white/60 border border-white/10 hover:bg-white/10'}`}>
