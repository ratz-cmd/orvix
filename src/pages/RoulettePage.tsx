...
type RouletteMediaFilter = 'all' | 'movie' | 'tv';
...
const fireConfetti = async () => {
  ...
  const colors = ['#3b82f6', '#f59e0b', '#10b981', '#3b82f6', '#8b5cf6', '#ec4899'];
...
const SpinningReel: React.FC<{
  ...
  return (
    <div ref={containerRef} className="relative w-full overflow-hidden rounded-2xl border border-white/10 bg-black/40 backdrop-blur-sm" style={{ height: CONTAINER_HEIGHT }}>
      {/* Center marker */}
      <div className="absolute left-1/2 top-2 bottom-2 z-20 pointer-events-none border-2 border-blue-500 rounded-xl shadow-[0_0_30px_rgba(239,68,68,0.3)]"
...
const VerticalReel: React.FC<{
  ...
  return (
    ...
    >
      <div
        className="absolute left-1 right-1 z-20 pointer-events-none border-2 border-blue-500 rounded-lg shadow-[0_0_20px_rgba(239,68,68,0.4)]"
    ...
    >
      ...
      {reelsVisible && (
        <div className="flex items-center justify-center gap-3 mb-3">
          <Dices className="w-5 h-5 text-blue-500 animate-spin" />
          <span className="text-sm font-semibold text-white/70">
            {t('roulette.roundProgress', { round })}
          </span>
          <div className="flex gap-1.5">
            <div className={`w-2 h-2 rounded-full transition-colors ${isRound1Done ? 'bg-green-500' : 'bg-blue-500 animate-pulse'}`} />
            <div className={`w-2 h-2 rounded-full transition-colors ${round === 2 ? 'bg-blue-500 animate-pulse' : 'bg-white/20'}`} />
          </div>
        </div>
      )}
      ...
      for (const type of types) {
        ...
        responses.forEach(res => {
          const items = res.data.results
            ...
            .map((item: any) => ({ ...item, media_type: type, is_anime: false }));
        ...
        <div className="max-w-screen-xl mx-auto">
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="flex flex-col sm:flex-row items-center justify-center gap-3 mb-8">
            ...
            <motion.button
              ...
              className="relative px-10 py-4 rounded-2xl bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-500 hover:to-blue-400 text-white font-bold text-lg shadow-lg shadow-red-500/25 disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center gap-3"
            >
              {loading ? (
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex justify-center gap-3 mb-6">
                ...
                {spinCount > 1 && (
                  <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                    onClick={skipAll}
                    className="px-4 py-2 rounded-xl bg-white/5 border border-white/10 hover:bg-blue-500/20 hover:border-blue-500/30 text-white/50 hover:text-white text-sm transition-all flex items-center gap-2"
                  >
                    <ChevronDown className="w-4 h-4" />
