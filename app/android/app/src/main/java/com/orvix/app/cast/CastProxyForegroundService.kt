...
import com.orvix.app.R
import com.orvix.app.proxy.CastMediaPreparer
import com.orvix.app.proxy.CronetMediaProxyUpstream
import com.orvix.app.proxy.MediaProxyPolicy
import com.orvix.app.proxy.MediaProxyServer
import com.orvix.app.proxy.MediaProxyServerConfig
import com.orvix.app.proxy.MediaProxySessionAccess
import com.orvix.app.proxy.MediaProxySessionStore
import com.orvix.app.proxy.NetworkBoundMediaProxyUpstream
...
class CastProxyForegroundService : Service() {
    ...
    private fun beginPreparation(pending: CastRelayPendingRequest) {
        ...
        if (selected == null) {
            pending.callback(
                Result.failure(IllegalStateException("ORVIX_RELAY_WIFI_ROUTE_REQUIRED")),
            )
        ...
        try {
            ...
            pending.callback(
                Result.failure(IllegalStateException("ORVIX_RELAY_START_FAILED")),
            )
            stopRelay(CastRelayStopReason.LOAD_FAILED)
        }
    }
    ...
    internal fun stopRelay(reason: CastRelayStopReason) {
        ...
        runCatching { sessionStore?.invalidateAll(com.orvix.app.proxy.MediaProxyMode.CAST_LAN) }
    ...
    private fun startVisibleForeground(deviceName: String) {
        ...
        val notification = builder
            ...
            .setContentTitle("Orvix diffuse vers $deviceName")
    ...
    companion object {
        const val ACTION_START = "com.orvix.app.cast.START_RELAY"
        const val ACTION_STOP = "com.orvix.app.cast.STOP_RELAY"
        const val EXTRA_REQUEST_ID = "requestId"
        private const val CHANNEL_ID = "orvix_cast_relay"
