...
import com.orvix.app.proxy.MediaProxyPolicy
import com.orvix.app.proxy.PreparedCastMedia
import com.orvix.app.proxy.PreparedCastTextTrack
...
internal class CastLoadCoordinator(
    ...
    fun load(
        ...
        finishPendingLoad(
            Result.failure(IllegalStateException("ORVIX_CAST_LOAD_REPLACED")),
        )
        loadSequence += 1L
                ...
                remoteLoadCallback@ { accepted ->
                    ...
                    if (accepted.isSuccess) {
                        ...
                        finishLoad(
                            sequence,
                            Result.failure(
                                IllegalStateException("ORVIX_CAST_LOAD_REJECTED"),
                            ),
                        )
                    }
                },
    ...
    private fun buildRemoteMedia(
        ...
        val orvixTransport = "android-lan-v1"
        val protocolVersion = 1
        val customData = JSONObject()
            .put("orvixTransport", orvixTransport)
    ...
    fun stop(callback: (Result<Unit>) -> Unit) {
        ...
        loadSequence += 1L
        finishPendingLoad(
            Result.failure(IllegalStateException("ORVIX_CAST_STOPPED")),
        )
        remote.stop { }
    ...
    private fun requireOpaqueLanUrl(rawUrl: String) {
        val uri = runCatching { URI(rawUrl) }
            .getOrElse { throw IllegalArgumentException("ORVIX_RELAY_INVALID_LAN_URL") }
        require(uri.scheme == "http" && uri.rawQuery == null && uri.fragment == null) {
            "ORVIX_RELAY_INVALID_LAN_URL"
        }
        val host = uri.host ?: throw IllegalArgumentException("ORVIX_RELAY_INVALID_LAN_URL")
