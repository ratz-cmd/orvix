...
import com.orvix.app.proxy.CastPreparedSource
import com.orvix.app.proxy.CastPreparedTextTrack
...
class CastModule internal constructor(
    ...
    private val sessionListener = object : SessionManagerListener<CastSession> {
        ...
        override fun onSessionEnded(session: CastSession, error: Int) {
            failPending("ORVIX_CAST_SESSION_ENDED")
            ...
            emit("CAST_SESSION_ENDED", Arguments.createMap().apply {
                putString("errorCode", "ORVIX_CAST_SESSION_ENDED")
            })
        }

        override fun onSessionStartFailed(session: CastSession, error: Int) {
            failPending("ORVIX_CAST_SESSION_FAILED")
            clearCoordinators(CastRelayStopReason.LOAD_FAILED)
            emitStatus(
                CastStatusMapper.disconnected("ORVIX_CAST_SESSION_FAILED"),
            )
            emit("CAST_SESSION_FAILED", Arguments.createMap().apply {
                putString("errorCode", "ORVIX_CAST_SESSION_FAILED")
            })
        }
        ...
        override fun onSessionResumeFailed(session: CastSession, error: Int) {
            clearCoordinators(CastRelayStopReason.SESSION_ENDED)
            emitStatus(
                CastStatusMapper.disconnected("ORVIX_RELAY_RELOAD_REQUIRED"),
            )
        }
            ...
            reactContext.runOnUiQueueThread {
                ...
                val errorCode = when (reason) {
                    CastRelayStopReason.NETWORK_LOST ->
                        "ORVIX_RELAY_NETWORK_LOST"
                    CastRelayStopReason.ADDRESS_CHANGED ->
                        "ORVIX_RELAY_ADDRESS_CHANGED"
                    CastRelayStopReason.NOTIFICATION_STOP ->
                        "ORVIX_RELAY_STOPPED"
                    else -> "ORVIX_RELAY_STOPPED"
                }
     ...
     * rejetait la lecture en ORVIX_CAST_PICKER_DISMISSED. D'où les appuis
    ...
    fun loadProxiedMedia(
        ...
        reactContext.runOnUiQueueThread {
            val parsed = runCatching {
                ...
                promise.reject("ORVIX_CAST_INVALID_SOURCE", "Invalid prepared Cast source")
            ...
            failPending("ORVIX_CAST_LOAD_REPLACED")
            pendingLoad = parsed
            runCatching {
                MediaRouteChooserDialog(activity).apply {
                    routeSelector = buildRouteSelector()
                    setOnDismissListener {
                        mainHandler.postDelayed({
                            ...
                            if (current?.isConnected != true && current?.isConnecting != true) {
                                failPending("ORVIX_CAST_PICKER_DISMISSED")
                ...
                failPending("ORVIX_CAST_PICKER_ERROR")
            }
        }
    }
        ...
        reactContext.runOnUiQueueThread {
            ...
            active.getStatus(refresh) { result ->
                result.fold(
                    ...
                    onFailure = {
                        promise.reject("ORVIX_CAST_STATUS_FAILED", "Unable to read Cast status")
                    },
                )
            }
        }
    ...
    fun stop(promise: Promise) {
        reactContext.runOnUiQueueThread {
            val pending = pendingCoordinator
            pendingCoordinator = null
            pending?.abandonPendingLoad("ORVIX_CAST_STOPPED")
            val active = acceptedCoordinator
            acceptedCoordinator = null
            if (active == null) {
    ...
    fun openBatterySettings(promise: Promise) {
        runCatching(userSettings::openBatterySettings).fold(
            onSuccess = { promise.resolve(true) },
            onFailure = {
                promise.reject("ORVIX_BATTERY_SETTINGS_UNAVAILABLE", "Settings unavailable")
            },
        )
    }
    ...
    private fun startLoad(session: CastSession, pending: PendingLoad) {
        ...
        if (receiverAddress == null || remote == null) {
            pending.promise.reject(
                "ORVIX_RELAY_RECEIVER_ADDRESS_REQUIRED",
            ...
            if (acceptedCoordinator == null) {
                ...
                emitAcceptedStatusOrError("ORVIX_RELAY_RECEIVER_ADDRESS_REQUIRED")
            }
            return
        }
        ...
        pendingCoordinator = null
        supersededPending?.abandonPendingLoad("ORVIX_CAST_LOAD_REPLACED")
                ...
                if (result.isSuccess) {
                    ...
                    emitAcceptedStatusOrError(
                        result.exceptionOrNull()?.message
                            ?.takeIf { it.startsWith("ORVIX_") }
                            ?: "ORVIX_CAST_LOAD_FAILED",
                    )
                }
                pending.promise.settle(result)
            },
    ...
    private fun parseMetadata(metadata: ReadableMap): CastRemoteMetadata {
        return CastRemoteMetadata(
            title = metadata.optionalString("title") ?: "Orvix",
    ...
    private fun withCoordinator(
        ...
        reactContext.runOnUiQueueThread {
            val active = acceptedCoordinator
            if (active == null) {
                promise.reject("ORVIX_CAST_NOT_LOADED", "No active prepared Cast media")
            } else {
                action(active)
            }
        }
    ...
    private fun Promise.settle(result: Result<Unit>) {
        result.fold(
            ...
            onFailure = {
                reject(
                    it.message?.takeIf { code -> code.startsWith("ORVIX_") }
                        ?: "ORVIX_CAST_COMMAND_FAILED",
                    "Cast command failed",
                )
            },
        )
    ...
    private fun CastRelayStopReason.toErrorCode(): String = when (this) {
        CastRelayStopReason.NETWORK_LOST -> "ORVIX_RELAY_NETWORK_LOST"
        CastRelayStopReason.ADDRESS_CHANGED -> "ORVIX_RELAY_ADDRESS_CHANGED"
        CastRelayStopReason.NOTIFICATION_STOP -> "ORVIX_RELAY_STOPPED"
        CastRelayStopReason.SESSION_ENDED -> "ORVIX_CAST_SESSION_ENDED"
        CastRelayStopReason.LOAD_FAILED -> "ORVIX_CAST_LOAD_FAILED"
        CastRelayStopReason.SERVICE_DESTROYED -> "ORVIX_RELAY_SERVICE_STOPPED"
        CastRelayStopReason.EXPLICIT -> "ORVIX_CAST_STOPPED"
    }
    ...
    private fun reloadRequiredStatus(deviceName: String?): NativeCastStatus {
        return CastStatusMapper.map(
            CastStatusSnapshot(
                ...
                errorCode = "ORVIX_RELAY_RELOAD_REQUIRED",
            ),
        )
    }
