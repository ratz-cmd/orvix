...
internal class CastMediaPreparer(
    ...
    private val executor: Executor = Executors.newSingleThreadExecutor { task ->
        Thread(task, "OrvixCastMediaPreparer").apply { isDaemon = true }
    },
) : Closeable {
    init {
    ...
    private fun inspectBoundedGet(
        ...
        response.use {
            ...
            require(looksLikeHls(contentType, bytes)) {
                throw CastMediaPreparationException("ORVIX_RELAY_UNSUPPORTED_MEDIA")
            }
            ...
            if (representativeUrl == null) {
                val profile = detectHlsProfile(
                    ...
                    ?: throw CastMediaPreparationException(
                        "ORVIX_RELAY_UNSUPPORTED_HLS_TRANSPORT",
                    )
    ...
    private fun prepareTextTrack(
        ...
        track.inlineVtt?.let { inlineVtt ->
            if (track.url != null) {
                throw CastMediaPreparationException("ORVIX_RELAY_INVALID_TEXT_TRACK")
            }
            return prepareInlineTextTrack(sessionId, track, inlineVtt)
        }
        val trackUrl = validateUrl(
            track.url ?: throw CastMediaPreparationException("ORVIX_RELAY_INVALID_TEXT_TRACK"),
        ).toString()
        ...
        response.use {
            ...
            val contentType = when {
                ...
                trackUrl.substringBefore('?').endsWith(".vtt", ignoreCase = true) -> "text/vtt"
                    ...
                    "ORVIX_RELAY_UNSUPPORTED_TEXT_TRACK",
                )
            }
    ...
    private fun prepareInlineTextTrack(
        ...
        val syntheticUrl = "https://inline.orvix.invalid/$digest.vtt"
    ...
    private fun execute(
        ...
        validateUrl(url)
        Log.i("OrvixCastDiag", "request method=$method url=$url")
        headers.forEach { (name, value) ->
            Log.i("OrvixCastDiag", "request_header name=$name value=$value")
        }
        ...
        response.headers.forEach { (name, value) ->
            Log.i("OrvixCastDiag", "response_header name=$name value=$value")
        }
        return response
    }
    ...
    private fun requireSuccess(response: MediaProxyUpstreamResponse) {
        if (response.statusCode !in 200..299) {
            throw CastMediaPreparationException("ORVIX_RELAY_UPSTREAM_ERROR")
        }
    }
    ...
    private fun readBounded(
        ...
        while (true) {
            ...
            total += count
            if (total > limit) {
                throw CastMediaPreparationException("ORVIX_RELAY_RESPONSE_TOO_LARGE")
            }
